...
- **规则**：配了规则服务时，这页只给规则服务管理页的链接（`destination.admin_url`），检索、修改已有规则在那里做。没配时：左栏六个规则集，右侧分「条目 / 体检」两个页签。条目可搜索、新增、移到另一类、删除（删除后底部出现「撤销」）。体检即 `tidy`：冗余可一键清理，保留域名可逐条删除，合并候选可一键「合并为 +.父域」（确认框提示合并后父域下所有子域都会归入该类；写入父域时被覆盖的子域自动移除）。
...
| `var/destination.json`（`destination.lock`） | 配了规则服务时：能用的线路（直连或经本机代理）、服务端条目的缓存（10 分钟重取）、已提交的决定与上线状态（结束后保留一天） |
...
- **本机文件**（默认）：上表的收件箱。
- **规则服务**：`settings.json` 里有 `destination`（格式见 `settings.example.json`）时，经写入接口直接写进自己维护的在线规则服务，不再写收件箱，也不要求 Clash 配置里有收件箱。`endpoint` 为写入接口，`key_file` 为写入密钥文件（只在本工具的 Python 进程里读，不进网页、不进网址），`rulesets` 指定每类写进服务端哪个规则集，`admin_url` 是服务端的管理页（规则页给链接）。
  ...
  - 访问服务端先试直连（8 秒），不通就经本机代理（`mixed-port`），记住能用的那条。
  - `status` 多一节「写入目的地」：连不连得上、密钥对不对、每类写进哪个规则集、Clash 里对不对得上、GitHub 令牌何时到期（服务端给出时）、最近几次提交的状态。
