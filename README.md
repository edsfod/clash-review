...
应用时记下的推荐来自当时的提示词，提示词改过之后就说明不了现在的提示词。改提示词后用 `python eval_decisions.py --requery`：按现在的提示词把全部裁定重问一遍（同一页面同一主机取最后一次决定），默认跑两遍（`--passes`），报告与逐项结果、所用提示词写到 `var/eval/requery-<日期时间>/`。两遍之间结论不同的项是模型自身的波动（2026-09-26：118 项里 9～13 项），一致率的变化小于这个幅度，说明不了提示词变好还是变坏；每遍都与你不同的项才值得看。重问不写网页的推荐缓存与身份缓存，也不把前后连接发给模型。

不作标准答案的裁定记在 `var/testcases/decisions_untrusted.json`：当时拿不准、照着模型推荐选的，拿来当标准答案只会把模型拉回它自己原先的结论。两种评法都不把它们算进一致率，单独列出模型的结论与你当时的选择；「忽略」也不计。格式：

```json
{"groups": [{"name": "证书状态", "why": "当时不懂", "patterns": ["lencr\\.org$"], "hosts": ["oneocsp.microsoft.com"]}]}
```

`patterns` 是匹配主机名的正则，`hosts` 是完整主机名。
...
| `var/testcases/decisions_untrusted.json` | 不作标准答案的裁定（组名、理由、正则或主机名），见第二节「人工裁定日志与评估」 |
...
- 2026-09-26（v1.3.4）：`eval_decisions.py` 加 `--requery`：按现在的提示词重问全部裁定，默认跑两遍估计模型自身的波动，报告、逐项结果与所用提示词存到 `var/eval/requery-<日期时间>/`；不作标准答案的裁定改为读 `var/testcases/decisions_untrusted.json`，两种评法都单独列出、不计入一致率。v1.3.3 的评估原先是临时脚本，方法并进了工具。
...
- 2026-09-26（v1.3.1）：「可改直连」的实测改为两段流水线：测速仍 6 个并发（测得准、不挤带宽），过了测速要问模型的交给另一个 12 并发的线程池。原先同一线程里测速与问模型串着做，模型一答几十秒，占住测速线程，60 项要好几分钟。配了规则服务时，页面上「写入 …」显示的是服务端的规则集名（如 common-direct），不再是收件箱名。
