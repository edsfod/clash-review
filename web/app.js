...
<dt>可改直连</dt><dd>走代理、但直连可能更好的主机（微软、苹果、Steam 的下载 CDN、证书吊销检查等），直连更快也省代理流量。候选按流量与次数排，去掉登录/账号类、AI 服务、同站你已归到代理的、拦截名单收录的。点「实测」过两道关：先直连与走代理各测 3 次首字节，直连连不上或不比代理快的隐藏；过了的再按策略问模型该不该直连——涉及登录、支付、个人数据或有地区限制的，按地区下发内容的（Google、微软的账号、商店、授权等）与被它们唤起、要同一出口的，可能被中间人攻击的（主流证书机构的证书状态服务、GitHub），都保持代理；学术站点分按 IP 认证的机构访问（直连）与要在境外登录的（代理）；出口 IP 探测、来历可疑的证书站点之类照样拉黑。策略全文见工具目录的 prompt.md。模型建议保持代理的也隐藏，页头可展开。「直连」写入 {{direct}}，「拉黑」写入 {{reject}}，「保持代理」记下来以后不再列出；测速与模型结论一直缓存，直到重新查询。</dd>
